...
--task compute_ranks_launcher(c_space        : ispace(int1d),
--                            r_pages        : region(Page),
--                            r_links        : region(Link(r_pages, r_pages)),
--                            p_links        : partition(disjoint, r_links, c_space),
--                            p_pages_dest   : partition(aliased, r_pages, c_space),
--                            p_pages_src    : partition(aliased, r_pages, c_space),
--                            damp           : double,
--                            num_pages      : uint64)

--where reads(r_pages.rank, r_pages.numLinks_out, r_links.src), reads writes(r_links.dest, r_pages.next_rank) do

--  for c in c_space do
    --compute_ranks(p_pages_dest[c], p_pages_src[c], p_links[c], damp, num_pages)
--  end
...
--  for col in c_space do
--    c.printf("\nlink subregion %d\n", col) 
--    for link in p_links[col] do
--      c.printf("color = %d\n", link.dest)
--    end
--  end

--  for col in c_space do
--    c.printf("\npage_dest subregion %d\n", col)
--    for page in p_pages_dest[col] do
--      c.printf("numLinks_out = %d\n", page.numLinks_out)
--    end
--  end

--  for col in c_space do
--    c.printf("\npage_src subregion %d\n", col)
--    for page in p_pages_src[col] do
--      c.printf("numLinks_out = %d\n", page.numLinks_out)
--    end
--  end
  ...
  --var p_pages_dest = partition(r_pages.color, c_space)
  --var p_links_dest = partition(r_links.dest_color, c_space)
  --var p_pages_src  = image(r_pages, p_links_dest, r_links.src)
  ...
  while not converged do
    ...
    --__demand(__parallel)
    --for c in c_space do
    --  compute_err(p_pages_dest[c], p_err[c])
    --end
    --converged = finish(r_err, config.error_bound)
